...
describe Bibliothecary::Parsers::Dub do
  ...
  it "parses dependencies from dub.json" do
    expect(described_class.analyse_contents("dub.json", load_fixture("dub.json"))).to eq({
      ...
      path: "dub.json",
      dependencies: [
        Bibliothecary::Dependency.new(name: "vibe-d", requirement: "~>0.7.22", type: "runtime", source: "dub.json"),
        Bibliothecary::Dependency.new(name: "libdparse", requirement: { "optional"=>true, "version"=>"~>0.2.0" }, type: "runtime", source: "dub.json"),
      ],
      kind: "manifest",
      success: true,
    })
  ...
  it "parses dependencies from dub.sdl" do
    expect(described_class.analyse_contents("dub.sdl", load_fixture("dub.sdl"))).to eq({
      ...
      path: "dub.sdl",
      dependencies: [
        Bibliothecary::Dependency.new(name: "vibe-d", requirement: "~>0.7.23", type: :runtime, source: "dub.sdl"),
      ],
      kind: "manifest",
      success: true,
    })
