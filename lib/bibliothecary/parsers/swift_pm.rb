  ...
  module Parsers
    class SwiftPM
      ...
      def self.parse_package_swift(manifest)
        ...
        raise Bibliothecary::ParseHostError.new("Http Error #{response.response_code} when contacting: #{Bibliothecary.configuration.swift_parser_host}/to-json", response.response_code) unless response.success?
