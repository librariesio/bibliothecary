  ...
  module Parsers
    class Carthage
      ...
      def self.map_dependencies(manifest, path)
        ...
        raise Bibliothecary::ParseHostError.new("Http Error #{response.response_code} when contacting: #{Bibliothecary.configuration.carthage_parser_host}/#{path}", response.response_code) unless response.success?
