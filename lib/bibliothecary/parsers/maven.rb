  ...
  module Parsers
    class Maven
      ...
      def self.parse_gradle(manifest)
        ...
        raise Bibliothecary::ParseHostError.new("Http Error #{response.response_code} when contacting: #{Bibliothecary.configuration.gradle_parser_host}/parse", response.response_code) unless response.success?
